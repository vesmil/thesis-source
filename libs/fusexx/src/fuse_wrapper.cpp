...
#warning truncate fuse_file_info unimplemented
    ...
    static int readdir(const char *pathname, void *buf,
                       fuse_fill_dir_t fillerDir, off_t off,
                       struct fuse_file_info *fi) {
        int flags = 0;
        detail::filler_handle = buf;
        detail::filler = fillerDir;
        return fuse().readdir(pathname, off, fi,
                              static_cast<FuseWrapper::readdir_flags>(flags));
    }
...
#warning utimens fuse_file_info unimplemented
    ...
    static int write_buf(const char *pathname, struct fuse_bufvec *buf,
    ...
    static int fallocate(const char *pathname, int mode, off_t offset,
