...
protected:
  ...
  enum readdir_flags {
     ...
     * FILL_DIR_FLAGS for the filler function.  The filesystem may also
   ...
   */
  int fill_dir(const std::string &name, const struct stat *stbuf, off_t off = 0,
   ...
   * Changed in version 2.2
   */
  virtual int write(const std::string &pathname, const char *buf, size_t count,
                    off_t offset, struct fuse_file_info *fi);
