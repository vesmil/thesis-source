...
#warning utimens fuse_file_info unimplemented
    ...
    static int utime(const char *pathname, struct utimbuf *times) {
        struct timespec tv[2] = {{.tv_sec = times->actime, .tv_nsec = 0},
...
int FuseWrapper::rename(const std::string &, const std::string &,
                        unsigned int) {
    return -ENOSYS;
}
...
int FuseWrapper::getxattr(const std::string &, const std::string &, char *,
                          size_t) {
    return -ENOSYS;
}
...
int FuseWrapper::fill_dir(const std::string &name, const struct stat *stbuf,
                          off_t off, FuseWrapper::fill_dir_flags flags) {
#if FUSE_VERSION < 30
    (void)flags;
...
int FuseWrapper::ioctl(const std::string &, int, void *,
...
int FuseWrapper::write_buf(const std::string &pathname,
                           struct fuse_bufvec *bufvec, off_t off,
                           struct fuse_file_info *fi) {
    int total = 0;
...
int FuseWrapper::read_buf(const std::string &pathname,
...
int FuseWrapper::main(int argc, char *argv[]) {
    ...
    static struct fuse_operations ops = {
        ...
        .flag_nullpath_ok = false, /* accept NULL pathnames, lets -ohard_remove
                                      work on unlinked files */
